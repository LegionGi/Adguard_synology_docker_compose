 ...
 <img width="100%" src="images/interface-setting.png" alt="synology-docker-compose interface-setting"/>
  ...
  Выберите в "Веб-интерфейс администрирования интерфейс" и "ДНС-сервер" - Сетевой интерфейс 192.168.1.200 (или Ваш IP), порты не меняем - придумываем Логин и пароль - переходим по 
 ссылке.
 Adguard Home будет доступен по адресу 192.168.1.200 (или Ваш IP) без указания порта.

 Может потребоваться создание отдельного нового подключения Bridge c автоматическим настройками в Container Manager
...
Через закладку Управление привязать к нашему проекту (поставить галочку).
